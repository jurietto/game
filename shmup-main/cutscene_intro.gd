...
func _ready() -> void:
    ...
    # --- Kitty image: smaller and high in the top center ---
    ...
    # Top center
    kitty_image.position = Vector2(screen.x * 0.5, screen.y * 0.11)
    # Smaller so it doesn't reach the text area
    kitty_image.custom_minimum_size = Vector2(screen.x * 0.25, screen.y * 0.18)
    ...
    var text_height := screen.y * 0.44
    # Start lower so it can’t overlap the kitty at all
    text_label.position = Vector2(margin_left, screen.y * 0.45)
